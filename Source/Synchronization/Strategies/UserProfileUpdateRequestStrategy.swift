...
public class UserProfileUpdateRequestStrategy: AbstractRequestStrategy, ZMSingleRequestTranscoder {
    ...
    public func request(for sync: ZMSingleRequestSync) -> ZMTransportRequest? {
        ...
        case self.phoneCodeRequestSync:
            ...
            return ZMTransportRequest(path: "/self/phone", method: .methodPUT, payload: payload, apiVersion: 0)
        ...
        case self.phoneUpdateSync:
            ...
            return ZMTransportRequest(path: "/activate", method: .methodPOST, payload: payload, apiVersion: 0)

        case self.phoneNumberDeleteSync:
            return ZMTransportRequest(path: "/self/phone", method: .methodDELETE, payload: nil, apiVersion: 0)

        case self.passwordUpdateSync:
            let payload: NSDictionary = [
                "new_password": self.userProfileUpdateStatus.passwordToSet!
            ]
            return ZMTransportRequest(path: "/self/password", method: .methodPUT, payload: payload, apiVersion: 0)

        case self.emailUpdateSync:
            let payload: NSDictionary = [
                "email": self.userProfileUpdateStatus.emailToSet!
            ]
            return ZMTransportRequest(path: "/access/self/email", method: .methodPUT, payload: payload, authentication: .needsCookieAndAccessToken, apiVersion: 0)

        case self.handleCheckSync:
            let handle = self.userProfileUpdateStatus.handleToCheck!
            return ZMTransportRequest(path: "/users/handles/\(handle)", method: .methodHEAD, payload: nil, apiVersion: 0)

        case self.handleSetSync:
            let payload: NSDictionary = ["handle": self.userProfileUpdateStatus.handleToSet!]
            return ZMTransportRequest(path: "/self/handle", method: .methodPUT, payload: payload, apiVersion: 0)
        ...
        case self.handleSuggestionSearchSync:
            ...
            return ZMTransportRequest(path: "/users/handles", method: .methodPOST, payload: payload, apiVersion: 0)
