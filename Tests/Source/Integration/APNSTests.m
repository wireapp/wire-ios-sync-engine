...
- (void)testThatItFetchesTheNotificationStreamWhenReceivingNotificationOfTypeNotice
{
    ...
    self.mockTransportSession.responseGeneratorBlock = ^ZMTransportResponse *(ZMTransportRequest *request) {
        ...
        if ([request.path isEqualToString:path] && request.method == ZMMethodGET) {
            ...
            return [ZMTransportResponse responseWithPayload:notificationStreamPayload HTTPStatus:200 transportSessionError:nil apiVersion:v0];
        };
        return nil;
    };
    ...
    self.mockTransportSession.responseGeneratorBlock = ^ZMTransportResponse *(ZMTransportRequest *request) {
        ...
        if ([request.path isEqualToString:path] && request.method == ZMMethodGET) {
            if (++requestCount == 2) {
                ...
                return [ZMTransportResponse responseWithTransportSessionError:NSError.tryAgainLaterError apiVersion:v0];
            }
        };
        return nil;
    };
