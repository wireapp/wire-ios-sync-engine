    ...
    func testThatItSendsARequestToUpdateSignalingKeys() {
        ...
        self.mockTransportSession.responseGeneratorBlock = { response in
            if response.path.contains("/clients/") && response.payload?.asDictionary()?["sigkeys"] != nil {
                didReregister = true
                return ZMTransportResponse(payload: [] as ZMTransportData, httpStatus: 200, transportSessionError: nil, apiVersion: .v0)
            }
            return nil
        }
        ...
        self.mockTransportSession.responseGeneratorBlock = { response in
            ...
            if response.path.contains("/clients/") && payload["sigkeys"] != nil {
                ...
                if tryCount == 0 {
                    ...
                    return ZMTransportResponse(payload: ["label": "bad-request"] as ZMTransportData, httpStatus: 400, transportSessionError: nil, apiVersion: .v0)
                }
                tryCount += 1
