...
extension ZMOperationLoopTests {
    func testThatMOCIsSavedOnSuccessfulRequest() {
        // given
        let request = ZMTransportRequest(path: "/boo", method: .methodGET, payload: nil, apiVersion: 0)
    ...
    func testThatMOCIsSavedOnFailedRequest() {
        ...
        // when
        request.complete(with: ZMTransportResponse(payload: nil, httpStatus: 400, transportSessionError: nil, apiVersion: 0))
        _ = waitForAllGroupsToBeEmpty(withTimeout: 0.5)
