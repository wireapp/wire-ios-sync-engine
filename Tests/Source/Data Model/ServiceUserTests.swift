...
final class ServiceUserTests: IntegrationTest {
    ...
    func testThatItDetectsBotRejectedResponse() {
        // GIVEN
        let response = ZMTransportResponse(payload: nil, httpStatus: 419, transportSessionError: nil, apiVersion: .v0)
        // WHEN
        let error = AddBotError(response: response)
        // THEN
    ...
    func testThatItDetectsBotNotResponding() {
        // GIVEN
        let response = ZMTransportResponse(payload: nil, httpStatus: 502, transportSessionError: nil, apiVersion: .v0)
        // WHEN
        let error = AddBotError(response: response)
        // THEN
